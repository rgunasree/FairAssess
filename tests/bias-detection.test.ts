...
test.describe('Bias Detection Tests', () => {
  ...
  test('should detect cultural bias', async () => {
    ...
    expect(result.fairnessScore).toBeLessThan(85)
  })
  ...
  test('should provide useful suggestions', async () => {
    const result = analyzeTextLocal('Must own a car and be a native English speaker.')
    expect(result.biasTypes).toContainEqual(expect.objectContaining({
      type: 'Socioeconomic Bias',
      suggestions: expect.arrayContaining(['reliable transportation']),
    }))
    expect(result.biasTypes).toContainEqual(expect.objectContaining({
      type: 'Cultural Bias',
