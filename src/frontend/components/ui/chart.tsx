...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [item] = payload
    const key = `${labelKey || (item && (item as any).dataKey) || (item && (item as any).name) || 'value'}`
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          const key = `${nameKey || (item as any).name || 'value'}`
          const itemConfig = getPayloadConfigFromPayload(config, item, key)
          const indicatorColor = (color as string | undefined) || (item as any).payload?.fill || (item as any).color
